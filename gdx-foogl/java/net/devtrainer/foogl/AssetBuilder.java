...
public class AssetBuilder {
	...
	public SpriteActor sprite(float x, float y, int width, int height, String key) {
		SpriteActor a = sprite(x, y, key, -1);
		TextureRegion r = texture(key);
		width = Math.min(width, r.getTexture().getWidth());
		height = Math.min(height, r.getTexture().getHeight());
		r.setRegionWidth(width);
		r.setRegionHeight(height);
		a.setTextureRegion(r);
		return a;
	}
	...
	public Scene getScene() {
		if (scene == null)
			scene = Game.defaultGameApp.getActiveScenes().first();
		return scene;
	}
